...
void ADungeonEscapeCharacter::Interact()
{
	...
	//DrawDebugSphere(GetWorld(), Start, InteractionSphereRadius, 20, FColor::Blue, false, 3.0f);
	DrawDebugSphere(GetWorld(), End, InteractionSphereRadius, 20, FColor::Blue, false, 3.0f);
	...
	if (HasHit)
	{
		...
		if (HitActor->ActorHasTag("CollectibleItem"))
		{
			...
			if (CollectibleItem)
			{
				Inventory.Add(CollectibleItem->ItemName);
		...
		else if (HitActor->ActorHasTag("Lock"))
		{
			...
			if (Lock)
			{
				...
				{
					int32 ItemRemoved = Inventory.RemoveSingle(Lock->KeyItemName);
					if (ItemRemoved)
					{
	...
	{
		UE_LOG(LogTemp, Display, TEXT("No Actor is hit"));
	}
}
